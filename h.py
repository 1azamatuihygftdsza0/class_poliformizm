...
ruyxat = {}
required_channel = "https://t.me/EMFliderlari"  # Kanal username sini kiriting
...
def kanal_button():
    ...
    button = InlineKeyboardButton("Kanalga qo‘shiling", url="https://t.me/EMFliderlari")  # Kanal URL sini kiriting
    markup.add(button)
    return markup
...
def tanishtiruv(message):
    ...
    if not check_channel_subscription(user_id):
        bot.send_message(
            message.chat.id,
            "Iltimos, kanalga qo‘shiling. Faqat shundan so‘ng botdan foydalanishingiz mumkin.",
            reply_markup=kanal_button())
        return
